...
-(void)hello:(GooeyCircle*)gooeyCircle andInteger:(NSInteger)index {
    if (index == 1) {
        
    }
    switch (index) {
        case 1:
             gooeyCircle.indicatorColor = [UIColor redColor];
            break;
        case 2:
            gooeyCircle.indicatorColor = [UIColor blueColor];
            break;
        case 3:
            gooeyCircle.indicatorColor = [UIColor grayColor];
            break;
        case 4:
            gooeyCircle.indicatorColor = [UIColor greenColor];
            break;
        case 5:
            gooeyCircle.indicatorColor = [UIColor blackColor];
            break;
        default:
            break;
    }
...
-(void)tapAction:(UITapGestureRecognizer *)ges{
    ...
    __block KYAnimatedPageControl *blockSelf = self;
    self.didSelectIndexBlock = ^(NSInteger index){
        [blockSelf hello:blockSelf->_gooeyCircle andInteger:index];
        _didSelectIndexBlock=nil;
    };
//    _gooeyCircle.indicatorColor = [UIColor blueColor];
...
//        NSLog(@"howmanydistance:%f",HOWMANYDISTANCE/self.pageCount);
        ...
        if(isFirst){
//            [self.indicator performSelector:@selector(restoreAnimation:) withObject:@(HOWMANYDISTANCE/self.pageCount) afterDelay:0];
