...
-(void)makeToastWithAlertViewBG:(NSString *)message duration:(CGFloat)interval position:(id)position title:(NSString *)title successOrNot:(BOOL)successOrNot
{
    UIImage *img;
    //todo
...
//    {
        if(successOrNot)
        {
            img=[UIImage imageNamed:@"alertViewBg_success.png"];
        }
        else
        {
            img=[UIImage imageNamed:@"alertViewBg_fail.png"];
        }
...
//        if(successOrNot)
//        {
//            img=[UIImage imageNamed:@"t1_alertViewBg_success.png"];
//        }
//        else
//        {
//            img=[UIImage imageNamed:@"t1_alertViewBg_fail.png"];
//        }
//    }
//    [self makeToast:message duration:interval position:position title:title image:img];
    UIView *toast=[self viewForMessage:message image:img];
    [self showToast:toast duration:interval position:position];
}
- (void)creatCorneView:(UIColor *)StrokeColor FillColor:(UIColor *)fillcolor{
    ...
    [maskLayer setLineWidth:1.0];
    maskLayer.fillColor = StrokeColor.CGColor;
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        ...
        } else if([point caseInsensitiveCompare:@"bottom2"] == NSOrderedSame) {
//todo
            //            if ([QDTool isDeviceiPad]) {
//                return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height-50 - (toast.frame.size.height / 2)) - CSToastVerticalPadding);
//            }
//            else {
                return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height-40 - (toast.frame.size.height / 2)) - CSToastVerticalPadding);
//            }
...
-(UIView*)viewForMessage:(NSString *)message image:(UIImage *)image
{
    ...
    UILabel *messageLabel=[QDTool createLabelWithFrame:CGRectMake(0, image.size.height/2+10, image.size.width, 20) fontColor:[UIColor whiteColor] fontSize:18 textAligment:NSTextAlignmentCenter];
    messageLabel.text=message;
    [imgView addSubview:messageLabel];
    return imgView;
}
...
//    UIView *wrapperView = [[[UIView alloc] init] autorelease];
    ...
    wrapperView.layer.cornerRadius = CSToastOpacity;
...
//    else
//    {
//        wrapperView.backgroundColor = [[QDTool hexStringToColor:@"#3b6f98"] colorWithAlphaComponent:1.0f];
//        wrapperView.layer.borderWidth=0.5f;
//        wrapperView.layer.borderColor=[QDTool hexStringToColor:@"3b6f98"].CGColor;
//    }
//    wrapperView.backgroundColor = [[UIColor clearColor] colorWithAlphaComponent:0];
...
//    else
//    {
//        messageLabel.textColor=[QDTool hexStringToColor:@"ffffff"];
//    }
