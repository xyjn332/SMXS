...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (scrollView.tag == TAG_DAYS) {
        if (index < _labelsDays.count) {
            [self highlightLabelInArray:_labelsDays atIndex:index];
        }else{
            [self highlightLabelInArray:_labelsDays atIndex:_labelsDays.count - 1];
        }
    } else if (scrollView.tag == TAG_MONTHS) {
        if (index < _labelsMonths.count) {
            [self highlightLabelInArray:_labelsMonths atIndex:index];
        }
        else{
            [self highlightLabelInArray:_labelsMonths atIndex:_labelsMonths.count - 1];
        }
    } else if (scrollView.tag == TAG_YEARS) {
        if (index < _labelsYears.count) {
            [self highlightLabelInArray:_labelsYears atIndex:index];
        }
        else{
            [self highlightLabelInArray:_labelsYears atIndex:_labelsYears.count - 1];
        }
...
- (void)updateSelectedDateAtIndex:(int)index forScrollView:(UIScrollView*)scrollView {
    ...
    if (scrollView.tag == TAG_DAYS) {
        ...
        if (_selectedYear > _curYear) {
            _selectedYear = _curYear;
        }
