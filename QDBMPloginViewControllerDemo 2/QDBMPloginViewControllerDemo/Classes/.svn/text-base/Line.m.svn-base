...
-(id)init{
    ...
    if (self) {
        ...
        self.shouldShowProgressLine = NO;
        self.pageCount = 6;
...
-(id)initWithLayer:(Line*)layer{
    ...
    if (self) {
        ...
        _selectedPage = layer.selectedPage;
        _lineHeight = layer.lineHeight;
        _ballDiameter = layer.ballDiameter;
        _unSelectedColor = layer.unSelectedColor;
        _selectedColor   = layer.selectedColor;
        _shouldShowProgressLine = layer.shouldShowProgressLine;
        _pageCount = layer.pageCount;
        _selectedLineLength = layer.selectedLineLength;
        _bindScrollView = layer.bindScrollView;
...
-(void)setSelectedPage:(NSInteger)selectedPage{
    if (_selectedPage != selectedPage) {
        ...
        _selectedLineLength = self.pageCount > 1 ? (selectedPage-1) * DISTANCE : 0;
...
-(void)drawInContext:(CGContextRef)ctx{
    ...
    if (self.shouldShowProgressLine == YES) {
        ...
        CGContextAddPath(ctx, linePath);
        CGContextSetFillColorWithColor(ctx, [UIColor lightGrayColor].CGColor);//self.selectedColor.CGColor);
        CGContextFillPath(ctx);
        CGPathRelease(linePath);
    }
...
//    CAKeyframeAnimation *anim = [[KYSpringLayerAnimation sharedAnimManager] createSpringAnima:@"selectedLineLength" duration:1.0 usingSpringWithDamping:0.5 initialSpringVelocity:3 fromValue:@(self.selectedLineLength) toValue:@(newLineLength)];
    ...
    CAKeyframeAnimation *anim = [[KYSpringLayerAnimation sharedAnimManager] createHalfCurveAnima:@"selectedLineLength" duration:0.6 fromValue:@(self.selectedLineLength) toValue:@(newLineLength)];
...
//    CAKeyframeAnimation *anim = [[KYSpringLayerAnimation sharedAnimManager] createBasicAnima:@"selectedLineLength" duration:0.2 fromValue:@(self.selectedLineLength) toValue:@(newLineLength)];
    ...
    _selectedLineLength = newLineLength;
    anim.delegate = self;
